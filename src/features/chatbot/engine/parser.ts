...
export function parseInput(raw: string): ParsedIntent {
  ...
  const topicMatch = CURRICULUM.find((t) =>
    input.includes(t.title.toLowerCase()) ||
  ...
  const FAQ_CATEGORIES = ['general', 'curriculum', 'pricing', 'placement', 'mentorship', 'schedule', 'technical', 'bonuses', 'enrollment', 'student support', 'about', 'careers'];
  ...
  if (input.includes('?') || input.startsWith('what') || input.startsWith('how') || input.startsWith('when') || input.startsWith('why') || input.startsWith('is ') || input.startsWith('does') || input.startsWith('can ') || input.startsWith('do ')) {
