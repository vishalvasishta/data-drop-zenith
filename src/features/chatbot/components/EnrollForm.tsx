      ...
      <div className="mb-4 flex items-center gap-1">
        {([1, 2, 3] as Step[]).map((s) => (
          <div key={s} className="flex flex-1 items-center gap-1">
            <div
              className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${step >= s ? "bg-violet-500" : "bg-white/10"}`}
            />
          </div>
        ))}
      </div>
            ...
            <Field label="Career Goal *" error={errors.careerGoal}>
              <div className="space-y-2">
                {GOAL_OPTIONS.map((g) => (
                  ...
                  >
                    <span
                      className={`h-4 w-4 rounded-full border-2 shrink-0 transition-colors ${data.careerGoal === g ? "border-violet-400 bg-violet-400" : "border-gray-500"}`}
                    />
                    {g}
                  </button>
                ))}
              ...
              {errors.careerGoal && (
                <p className="mt-1 text-xs text-red-400">{errors.careerGoal}</p>
              )}
            </Field>
          )}
        </motion.div>
